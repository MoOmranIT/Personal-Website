export const siteConfig = {
  name: 'Dr. Khaled Al Mohammad',
  title: 'Dr. Khaled Al Mohammad | Business Consultant in Dubai — Strategy, Marketing & Growth',
  description: 'Dr. Khaled Al Mohammad is a Dubai-based Business Consultant with 18+ years of experience in business development, digital marketing and strategic growth. PhD in Marketing, author of 6+ books, trainer of 3,000+ professionals. Book your consultation today.',
  ogDescription: '18+ years of experience in business consulting, strategic marketing and growth development. Book your consultation with Dr. Khaled Al Mohammad today.',
  twitterDescription: 'Strategic business consulting, marketing expertise and growth solutions. 18+ years of experience across the Gulf.',
  url: 'https://www.drkhaledalmohamad.com/',
  locale: 'en_US',
  author: 'Dr. Khaled Al Mohammad',
  contact: {
    email: 'info@dr-khaledalmohamad.com',
    phone: '+971553433150',
    whatsapp: 'https://wa.me/971553433150',
    address: 'Dubai, United Arab Emirates'
  },
  social: {
    linkedin: '#',
    instagram: '#',
    twitter: '#',
    amazon: 'https://www.amazon.com/stores/Dr.-Khaled-Almohamad/author/B0H217J18V'
  },
  placeholder: {
    phone: '+971 55 343 3150',
    whatsapp: '971553433150'
  },
  contactApiUrl: import.meta.env.PUBLIC_CONTACT_API_URL || '/api/contact',
  seo: {
    en: {
      title: 'Dr. Khaled Al Mohammad | Business Consultant in Dubai — Strategy, Marketing & Growth',
      description: 'Dr. Khaled Al Mohammad is a Dubai-based Business Consultant with 18+ years of experience in business development, digital marketing and strategic growth. PhD in Marketing, author of 6+ books, trainer of 3,000+ professionals. Book your consultation today.',
      ogDescription: '18+ years of experience in business consulting, strategic marketing and growth development. Book your consultation with Dr. Khaled Al Mohammad today.',
      twitterDescription: 'Strategic business consulting, marketing expertise and growth solutions. 18+ years of experience across the Gulf.',
      locale: 'en_US'
    },
    ar: {
      title: 'د. خالد المحمد – لكل شركة قصة نمو تستحق أن تُروى | مستشار أعمال في دبي',
      description: 'د. خالد المحمد هو مستشار أعمال في دبي مع أكثر من 18 عاماً من الخبرة في تطوير الأعمال، الاستشارات التسويقية، والتدريب. لكل شركة قصة نمو تستحق أن تُروى.',
      ogDescription: 'اكتشف كيف يمكن لخبرة 18 عاماً في الاستشارات الاستراتيجية أن تحوّل نمو شركتك. د. خالد المحمد — مستشار أعمال في دبي.',
      twitterDescription: 'استشارات الأعمال الاستراتيجية، خبرة تسويقية، وبرامج تدريبية. أكثر من 18 عاماً في دبي والخليج.',
      locale: 'ar_AR'
    }
  }
};
